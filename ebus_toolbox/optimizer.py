...
def greedy_optimization(electrified_stations, electrified_station_set, new_scen, new_sched,
                        not_possible_stations, soc_upper_thresh=1, soc_lower_thresh=0, **kwargs):
...
def group_optimization_quick(group, base_scen, base_sched,
    ...
    if brute:
        ...
        best_station_ids = choose_station(station_eval, electrified_station_set,
                                          pre_optimzed_set, decision_tree,
                                          missing_energy=missing_energy)
    ...
    if decision_tree is not None:
        try:
            decision_tree[str(electrified_station_set)]["missing_energy"] = delta_energy
            decision_tree[str(electrified_station_set)]["visit_counter"] += 1
            logger.debug("already visited")
        except KeyError:
            decision_tree[str(electrified_station_set)] = dict()
            decision_tree[str(electrified_station_set)]["missing_energy"] = delta_energy
            decision_tree[str(electrified_station_set)]["visit_counter"] = 1
...
def choose_station_brute(station_eval, electrified_station_set,
    ...
    for comb in a:
        if str(set(comb)) not in decision_tree:
            return comb
    else:
...
def choose_station(station_eval, electrified_station_set,
                   pre_optimzed_set=None, decision_tree=None, missing_energy=0):
    ...
    if pre_optimzed_set is not None:
        ...
        if pot <= -missing_energy:
            print("Not enough potential after ", electrified_station_set)
            return None
    ...
    for station in station_eval:
        ...
        if decision_tree is not None:
            if str(check_stations) in decision_tree.keys():
                min_count_visited = min(min_count_visited,
                                        decision_tree[str(check_stations)]["visit_counter"])
        ...
        for station in station_eval:
            # Create a station combination from already electrified stations and possible new station
            check_stations = electrified_station_set.union([station[0]])
            if decision_tree[str(check_stations)]["visit_counter"] == min_count_visited:
                best_station_id = station[0]
                return [best_station_id]
    return None
...
#
# def timeseries_calc_folded_dep(station, events, soc_dict_folded, eval_scen):
#     soc_dict_folded = copy(soc_dict_folded)
#     charging_curve = [[0, 450], [0.8, 296], [0.9, 210], [1, 20]]
#     soc_over_time_curve = charging_curve_to_soc_over_time(charging_curve, BATTERY_CAPACITY)
#     for e in events:
#         if station in e["stations"]:
#             for i, trip in enumerate(e["trip"]):
#                 if trip.arrival_name != station:
#                     continue
#                 try:
#                     idx = np.argwhere(soc_dict_folded[e["vehicle_id"]][:,0]==trip.arrival_time)[0][0]
#                 except:
#                     continue
#                 soc = soc_dict_folded[e["vehicle_id"]][:,2]
#                 try:
...
#                 standing_time = standing_time / datetime.timedelta(minutes=1)
#                 d_soc = get_delta_soc(soc_over_time_curve, soc[idx], standing_time)
#                 soc[idx:] += d_soc
#                 soc_max = np.max(soc)
#                 while soc_max > 1:
...
def get_index_by_time(search_time, this_scen: scenario.Scenario):
    ...
    delta_time = datetime.timedelta(minutes=60 / this_scen.stepsPerHour)
...
#
# def get_below_zero_soc_events_folded_(this_scen: scenario.Scenario, rotations,
...
#         rot = this_sched.rotations[rot_id]
#         data = this_scen.vehicle_socs_folded[rot.vehicle_id]
#         soc = data[:, 2][(data[:, 0] >= rot.departure_time) * (data[:, 0] <= rot.arrival_time)]
#
#         # soc, rot_start_idx, rot_end_idx = get_rotation_soc(rot_id, this_sched, this_scen)
#         # soc = [s if s is not None else 999 for s in soc]
#         idx = range(0, len(soc))
#
#         comb = list(zip(soc, idx))
...
#             i = min_idx
#             while soc[i] < SOC_UPPER_THRESH:
#                 if i >= len(soc) - 1: break
#                 i += 1
#             end = idx.index(i)
#
#             possible_stations = set(data[start:min_idx, 1])
#
#             trips = set(data[start:min_idx, 3])
#
